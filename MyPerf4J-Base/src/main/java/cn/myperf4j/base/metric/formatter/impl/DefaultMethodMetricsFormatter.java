...
public final class DefaultMethodMetricsFormatter implements MethodMetricsFormatter {
    ...
    public String format(List<MethodMetrics> methodMetricsList, long startMillis, long stopMillis) {
        ...
        String dataTitleFormat = "%-" + maxApiLength + "s%13s%13s%9s%9s%9s%9s%9s%10s%9s%9s%9s%9s%9s%9s%n";
        ...
        sb.append(String.format(dataTitleFormat, "Method[" + methodMetricsList.size() + "]", "Type", "Level", "RPS", "Avg(ms)", "Min(ms)", "Max(ms)", "StdDev", "Count", "TP50", "TP90", "TP95", "TP99", "TP999", "TP9999"));
        if (methodMetricsList.isEmpty()) {
            return sb.toString();
        }

        String dataFormat = "%-" + maxApiLength + "s%13s%13s%9d%9.2f%9d%9d%9.2f%10d%9d%9d%9d%9d%9d%9d%n";
